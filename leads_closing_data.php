...
<?php
		$i = 1;
        $jml_per_halaman = 20;
        $jml_data = mssql_num_rows(mssql_query("SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,nama_nasabah,produk,nominal_pengajuan,tgl_input,ket FROM cart where ket=4 ) AS a 
        ...
        $jml_halaman = ceil($jml_data / $jml_per_halaman);
				if($_SESSION['user_level']==1 || $_SESSION['user_level']==2 )
				{
						...
						$query = mssql_query("SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,nama_nasabah,produk,nominal_pengajuan,tgl_input,ket FROM cart where ket=4 ) AS a 
												LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
												LEFT JOIN area d on c.id_area=d.id_area
				...
				}elseif($_SESSION['user_level'] == 6||$_SESSION['user_level']==7)
				{
						...
						$kunci = $_POST['cari'];
						echo "<strong>Hasil pencarian untuk kata kunci $kunci</strong>";
						$query = mssql_query(" SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,id_user_atasan,nama_nasabah,produk,nominal_pengajuan,tgl_input,ket FROM cart where ket=4 and id_user_atasan='$_SESSION[npp]' ) AS a 
													LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
													LEFT JOIN area d on c.id_area=d.id_area
											WHERE a.nama_nasabah LIKE '%$kunci%'
											OR a.npp LIKE '%$kunci%'
				...
				}elseif($_SESSION['user_level'] == 8 || $_SESSION['user_level'] == 9 || $_SESSION['user_level'] == 11 || $_SESSION['user_level'] == 12)
				{
						...
						$kunci = $_POST['cari'];
						echo "<strong>Hasil pencarian untuk kata kunci $kunci</strong>";
						$query = mssql_query("SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,nama_nasabah,produk,nominal_pengajuan,tgl_input,ket FROM cart where ket=4 and npp='$_SESSION[npp]') AS a 
												LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
												LEFT JOIN area d on c.id_area=d.id_area
											WHERE a.nama_nasabah LIKE '%$kunci%'
											OR a.npp LIKE '%$kunci%'
...
?>	
  ...
  <?php if(mssql_num_rows($query)>0){ ?>
        ...
        <tr>
            ...
            <td><a href="index.php?page=29g&id=<?php echo $data['id']; ?>" class="btn">view</a></td>
		</tr>
        <?php $no++; } ?>
        <?php } ?>
    </table>
