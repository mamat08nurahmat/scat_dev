...
<table class="table table-condensed table-bordered table-hover" cellpadding="0" cellspacing="0">
    ...
    <?php 
        $i = 1;
        $jml_per_halaman = 20;
        $jml_data = mssql_num_rows(mssql_query("SELECT * FROM ( SELECT id_report,ROW_NUMBER() OVER (ORDER BY nama_nasabah ASC) AS ROWNUM,nama_nasabah,persetujuan_pengalihan,nominal,no_perjanjian,no_rekening,keterangan,id_cabang,ket,tgl_update,outstanding
												FROM report_tl where persetujuan_pengalihan='2' ) AS a  
	...
	if($_SESSION['user_level']==1||$_SESSION['user_level']==2) 
	  {
        ...
        if(isset($_POST['cari3'])) {
            ...
            $query = mssql_query("SELECT * FROM ( SELECT id_report,ROW_NUMBER() OVER (ORDER BY nama_nasabah ASC) AS ROWNUM,nomor_pinjaman,no_rekening_affiliasi,nama_nasabah,persetujuan_pengalihan,nominal,no_perjanjian,no_rekening,keterangan,id_cabang,ket,tgl_update,outstanding
											FROM report_tl where persetujuan_pengalihan='2' ) AS a  
											LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang =  'KCU') c on a.id_cabang=c.kode_cabang LEFT JOIN area d on c.id_area=d.id_area
                WHERE a.nama_nasabah LIKE '%$kunci%'
                OR a.persetujuan_pengalihan LIKE '%$kunci%'
                OR a.keterangan LIKE '%$kunci%'
	...
	elseif($_SESSION['user_level']==7) 
	{
		 ...
		 if(isset($_POST['cari3'])) {
            ...
            echo "<strong>Hasil pencarian untuk kata kunci $kunci</strong>";
            $query = mssql_query("
               SELECT * FROM ( SELECT id_report,ROW_NUMBER() OVER (ORDER BY nama_nasabah ASC) AS ROWNUM,nomor_pinjaman,no_rekening_affiliasi,nama_nasabah,persetujuan_pengalihan,nominal,no_perjanjian,no_rekening,keterangan,id_cabang,ket,tgl_update,outstanding
											FROM report_tl where persetujuan_pengalihan='2' and id_cabang='$_SESSION[id_cabang]' ) AS a  
											LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang =  'KCU') c on a.id_cabang=c.kode_cabang LEFT JOIN area d on c.id_area=d.id_area
                WHERE a.nama_nasabah LIKE '%$kunci%'
                OR a.persetujuan_pengalihan LIKE '%$kunci%'
                OR a.keterangan LIKE '%$kunci%'
