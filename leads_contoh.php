...
<?php
	...
	if($_SESSION['user_level']==1 || $_SESSION['user_level']==2 )
	{
		 if(isset($_POST['cari'])) {
            ...
            $query = mssql_query("
               select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where ket=1) AS a 
...
<?php
	...
	elseif($_SESSION['user_level'] == 6||$_SESSION['user_level']==7)
	{
	 if(isset($_POST['cari'])) {
            ...
            echo "<strong>Hasil pencarian untuk kata kunci $kunci</strong>";
            $query = mssql_query("
               select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where ket=1 and id_cabang='$_SESSION[id_cabang]') AS a 
									LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
									LEFT JOIN area d on c.id_area=d.id_area 
				WHERE a.nama_prospek LIKE '%$kunci%'
                OR c.nama_cabang LIKE '%$kunci%'
        ...
        // query jika nomor halaman sudah ditentukan
            ...
            $halaman = $_POST['halaman'];
            $i = ($halaman - 1) * $jml_per_halaman  + 1;
			$query = mssql_query("select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where npp is null and ket=1 and id_cabang='$_SESSION[id_cabang]') AS a 
									LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
									LEFT JOIN area d on c.id_area=d.id_area 
									WHERE a.ROWNUM BETWEEN (($halaman - 1) * $jml_per_halaman)+1 AND ($jml_per_halaman*$halaman) ORDER BY a.nama_prospek DESC");
		...
		?>
<form method="post" action="proses_masuk_kecart_spv.php">	
...
<?php
	...
	if(isset($_POST['cari'])) {
        ...
        // query jika nomor halaman sudah ditentukan
            ...
            $halaman = $_POST['halaman'];
            $i = ($halaman - 1) * $jml_per_halaman  + 1;
			$query = mssql_query("select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where ket=1 and id_cabang='$_SESSION[id_cabang]') AS a 
									LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
									LEFT JOIN area d on c.id_area=d.id_area 
									WHERE a.ROWNUM BETWEEN (($halaman - 1) * $jml_per_halaman)+1 AND ($jml_per_halaman*$halaman) ORDER BY a.nama_prospek DESC");
		...
		?>
	<form method="post" action="proses_masuk_kecart.php">		
	<div class="pull-right">
		<input type="submit" name="PILIH" value="PILIH"><br><br>
	</div>
	<table class="table table-condensed table-bordered table-hover" cellpadding="0" cellspacing="0">
