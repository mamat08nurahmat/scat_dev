...
<tbody>
    <?php 
        ...
        $jml_data = mssql_num_rows(mssql_query("SELECT * FROM (
																...
																FROM sales_penyelia where id_grup in (8,9,11) and id_user_atasan='$_SESSION[npp]'
	   ...
	   // query pada saat mode pencarian
        if(isset($_POST['cari'])) {
            ...
            $query = mssql_query(" SELECT * FROM ( select * from sales_penyelia where id_grup in (8,9,11) and id_user_atasan='$_SESSION[npp]' and status_sales='1')As a LEFT JOIN app_user_grup b on a.id_grup = b.id_grup LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang LEFT JOIN area d on c.id_area=d.id_area
