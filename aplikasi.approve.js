(function($) {
	...
	$(document).ready(function(e) {
		...
		$('.ubah, .tambah').live("click", function(){
			...
			var url = "sales-form-approve.php";
			kd_npp = this.id;
		...
		// ketika tombol ubah/tambah ditekan
		$('.update_cancel').live("click", function(){

			var url = "sales-input-approve.php";
			kd_npp = this.id;
				$("#kd_npp").val(kd_npp);
		});
		...
		// ketika tombol simpan ditekan
		$("#simpan-approve").bind("click", function(event) {
			var url = "sales-input-cancel.php";

			// mengambil nilai dari inputbox, textbox dan select
			var v_npp = $('input[name=kd_npp]').val();
			var v_keterangan = $('textarea[name=keterangan]').val();
			// mengirimkan data ke berkas transaksi.input.php untuk di proses
			$.post(url, {kd_npp: v_npp, keterangan: v_keterangan } ,
						 function() {
				// tampilkan data mahasiswa yang sudah di perbaharui 
				// ke dalam <div id="data-mahasiswa"></div>
				$("#data-mahasiswa").load(main);
