...
header("Content-Disposition: attachment; filename=leads_closing.xls");
$i = 1;
if($level==1||$level==2)
{
	$sql=mssql_query("SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,nama_nasabah,produk,nominal_pengajuan,no_aplikasi,tgl_update,ket FROM cart where ket=4 ) AS a 
...
{
	$sql=mssql_query("SELECT * FROM ( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,npp,id_cabang,nama_nasabah,produk,nominal_pengajuan,no_aplikasi,tgl_update,ket FROM cart where ket=4 and id_cabang='$_SESSION[id_cabang]' ) AS a 
												LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
												LEFT JOIN area d on c.id_area=d.id_area
				");
}
...
?>
		<tr>
			...
			<td><font color='black'><center><?php echo $data['no_aplikasi']; ?></center></font></td>
