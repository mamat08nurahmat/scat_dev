...
/*
	$folder 	= "uploads/scan/";
	$eror		= false;
	$file_type	= array('jpg','jpeg','pdf');
	$max_size	= 7000000; // 1MB
	...
	$extensi2	= $explode2[count($explode1)-1];
	
	$cek_username = mssql_num_rows(mssql_query("SELECT * FROM sales WHERE no_ktp='$no_ktp'"));		
	if ($cek_username > 0){
	echo "
	<script> 
		alert('Username sudah ada yang pakai. Ulangi lagi');
		window.location.replace('index.php?page=10c');
	</script>";
	}
	else{
	*/
...
/*	if(!in_array($extensi,$file_type)){
	...
	if($eror == true){
		...
		if(move_uploaded_file($_FILES['upload']['tmp_name'], $folder.$file_name)){
				if(move_uploaded_file($_FILES['komitmen']['tmp_name'], $folder.$file_name2)){
			*/	
			...
			$aprove = mssql_query("update contoh set ket='$ket',tgl=SYSDATETIME(),grade='3' where id='$kd_npp'") ;
			$simpan = mssql_query("INSERT INTO sales (npp,id_grup,id_cabang,nama,no_ktp,tanggal_lahir,alamat,grade,telepon,tanggal_aktif,pass,id_vendor)
				SELECT			  (select max(npp)+1 from sales where id_grup in(8,9,11,12)),id_grup,id_cabang,nama_lengkap,no_ktp,tanggal_lahir,alamat,grade,hp,tanggal,dbo.md5('bankbni'),id_vendor
				FROM contoh a 
				join history_sales e on a.id=e.id
				where a.id='$kd_npp'and e.ket='5';");
			if($simpan && $aprove && $order){
			echo" <script> 
				alert('DATA SUKSES DITAMBAHKAN');
				window.location.replace('index.php?page=10e');
				</script>";	
			} else{
				
			echo " DATA GAGAL DI TAMBAHKAN ".mssql_get_last_message();
				}
			}
?>
...
?>
	...
	<table align = "center">
		<tr><td><select name="ket">
		<option <?php if( $ket=='5'){echo "selected"; } ?> value='5'>Approve</option>
		<option <?php if( $ket=='4'){echo "selected"; } ?> value='4'>cancel</option>
		</td></tr>
		<tr><td>
		<textarea id="keterangan" name="keterangan"  type="text" class="required" required></textarea>
		</td></tr>
	</table>
		...
		<tr>
			...
			<a href="index.php?page=10e" class="btn btn-danger" >kembali</a></center>
			</td>
		</tr>
