...
$(document).ready(function(){
	...
	var month=$("#month").val();
	var year=$("#year").val();
	var x="tampil_realisasi.php?id="+nnp +"&month="+month+"&year="+year;
	$("#kupret").load(x);
}
);
});
...
$(document).ready(function(){
	...
	var month=$("#month").val();
	var year=$("#year").val();
	var x="report-lending.php?id="+nnp +"&month="+month+"&year="+year;
	$("#kupret").load(x);
}
);
});
...
<!--<form method="POST" action="tampil.php">-->
		<table>
			<div class="form_settings">
				...
				?>
					<div class="span12">
					<span>NPP</span><span>:</span><input  name="npp" id="npp" type="text" value="<?php echo $_SESSION['username'] ?>" readonly="readonly" style="width:168px;"/>
					</div>
					<div class="span12">
					<span>NAME</span><span>:</span><input  name="nama" id="nama" type="text" value="<?php echo $_SESSION['namauser'] ?>" style="height:20px;" readonly="readonly"/>
					</div>
					<div class="span12">
					<span>TYPE</span><span>:</span><input  name="sales_type" id="sales_type" value="<?php echo $_SESSION['nama_grup'] ?>" type="text" style="height:20px;" readonly="readonly"/>
					</div>
					<div class="span12">
				...
				<span>TAHUN</span><span>:</span><select name="year" id="year" type="text" class="form-control" style="border:1px solid #E6E6FA; padding: 6px 2px; width: 180px;">
             	...
             	<div class="span12"><span>&nbsp;</span>				  
				<button class="btn">Hitung</button>
				<style>
				.tombol {
				  background: #ffffff;
				  background-image: -webkit-linear-gradient(top, #ffffff, #e1e4e6);
				  background-image: -moz-linear-gradient(top, #ffffff, #e1e4e6);
				  background-image: -ms-linear-gradient(top, #ffffff, #e1e4e6);
				  background-image: -o-linear-gradient(top, #ffffff, #e1e4e6);
				  background-image: linear-gradient(to bottom, #ffffff, #e1e4e6);
				...
				.tombol:hover {
				  background: #f0f3f5;
				  background-image: -webkit-linear-gradient(top, #f0f3f5, #d4d8db);
				  background-image: -moz-linear-gradient(top, #f0f3f5, #d4d8db);
				  background-image: -ms-linear-gradient(top, #f0f3f5, #d4d8db);
				  background-image: -o-linear-gradient(top, #f0f3f5, #d4d8db);
				  background-image: linear-gradient(to bottom, #f0f3f5, #d4d8db);
				  text-decoration: none;
				}
				</style>
				<span>&nbsp;</span><button class="tombol">DETAIL</button>
				</div>
				<div class="span12">
				<br>
				</div>
