...
header("Content-Disposition: attachment; filename=leads_store.xls");
$i = 1;

if($level==1||$level==2)
{
	$sql=mssql_query("select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where ket=1) AS a 
...
{
	$sql=mssql_query("select * from( SELECT id,ROW_NUMBER() OVER (ORDER BY id) AS ROWNUM,id_cabang,nama_prospek,produk,nominal_pengajuan,tgl_input,ket FROM leads where ket=1 and id_cabang='$_SESSION[id_cabang]') AS a 
										LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang 
										LEFT JOIN area d on c.id_area=d.id_area 
					");
}
...
<table width="663" border="0">
  <tr>
   ...
   <strong>LEADS MART STORE SALES COMPANY <br> PT. Bank Negara Indonesia </strong>
   </span>
   </td>
