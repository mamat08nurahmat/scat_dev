...
<tbody>
    <?php 
        ...
        if(isset($_POST['cari'])) {
            ...
            $query = mssql_query("
                SELECT * FROM sales a LEFT JOIN app_user_grup b on a.id_grup = b.id_grup LEFT JOIN (SELECT KODE_cabang,ID_AREA,nama_cabang FROM CABANG WHERE tipe_cabang = 'KCU') c on a.id_cabang=c.kode_cabang LEFT JOIN area d on c.id_area=d.id_area
    ...
    <tr>
		...
		<td><button onclick="reset('<?php echo $data['npp'];?>')">Reset</button></td>
    </tr>
    <?php
        $i++;
        }
